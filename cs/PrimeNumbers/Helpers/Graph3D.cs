...
namespace PrimeNumbers.Helpers {
    ...
    public class Graph3D : UserControl {
        ...
        cPoint3D[,] mi_PolyArr;
        List<cScatter> mi_ScatterArr = new List<cScatter>();
        cMinMax3D mi_MinMax;
        ...
        public Color[] ColorScheme {
            set {
                Debug.Assert(!InvokeRequired); // Do not call from other threads or use Invoke()
                mi_SchemeBrushes = new SolidBrush[value.Length];
                for (int i = 0; i < mi_SchemeBrushes.Length; i++) {
                    mi_SchemeBrushes[i] = new SolidBrush(value[i]);
                }
                Invalidate(); // repaint only
            }
        }
        ...
        public void SetScatterPoints(List<cScatter> scatterPoints, eNormalize e_Normalize) {
            ...
            mi_PolyArr = null;
            mi_ScatterArr = scatterPoints;

            ms32_Points = scatterPoints.Count;
            mi_MinMax = new cMinMax3D(scatterPoints.ToArray());
        ...
        private object _graph3DLock = new object();

        private void CreateScatterDots() {
            var scatterPlots = new List<cScatter>();

            lock (_graph3DLock) {
                scatterPlots.AddRange(mi_ScatterArr);
            }

            // Convert 3D --> 2D
            foreach (cScatter i_Scatter in scatterPlots) {
        ...
        private Brush GetSchemeBrush(double d_FactorZ) {
            ...
            int s32_Brush = (int)(d_FactorZ * (mi_SchemeBrushes.Length - 1));
            return mi_SchemeBrushes[s32_Brush];
        }
