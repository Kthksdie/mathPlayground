...
namespace TestingNumbers.Helpers {
    static class WindowUtility {
        ...
        public static void MoveWindowToCenter() {
            IntPtr window = GetConsoleMainWindowHandle();
        ...
        public static IntPtr GetConsoleMainWindowHandle() {
            var currentProcess = Process.GetCurrentProcess();

            foreach (Process processes in Process.GetProcesses()) {
                if (processes.MainWindowTitle.Contains(currentProcess.ProcessName)) {
                    if (processes.ProcessName == "WindowsTerminal") {
                        return processes.MainWindowHandle;
                    }
                }
            }
